...
def append_subscription(
    ...
    if SHEETS_WEBAPP_URL:
        ...
        logger.info("Sending to Google Sheets: notify_days='%s' (type: %s)", notify_days, type(notify_days).__name__)
        if _send_to_webapp("subscription", data):
            logger.info("Written to Subscriptions (Web App): user_id=%s, notify_days='%s'", user_id, notify_days)
            return True
